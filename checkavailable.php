...
$con = new mysqli('localhost', 'root', 'manager', 'secdev', 3307);
    ...
    if(isset($_POST['snoEdit'])){
        ...
        $id = $_POST['id'];
        $cid = $_POST['cid'];   
         $sql = "UPDATE `personnel` SET  `Client_ID` = '$cid'  WHERE `personnel`.`ID` = '$id';";
  ...
  <body>
    ...
    <div class="header">
      <div class="child1">
        ...
        <p class="p">Available Personnel Details</p>
        <br>
      </div>
      <!-- <div class="child2">
        <a href="/dmp/"><button class="btn btn-sm btn-primary">Check Available</button></a>
      </div> -->
    </div>  
    ...
    <table class="table myTable" id="myTable">
      ...
      <tbody>
          <?php
      $sql = "SELECT * FROM `personnel` WHERE `Client_ID`= 0";
      $sno=0;
    $result = mysqli_query($con, $sql);
    ...
    <div class="modal-content">
      ...
      <form action="checkavailable.php" method="post">
          ...
          <label for="text" id="idd" style="color:black;">ID</label>
          <input type="text" name="id" id="id" placeholder="Enter ID" >
            <input type="text" name="nme" id="nme" placeholder="Enter Name" disabled>
           <label for="text" style="color:black;">Client_ID</label>
