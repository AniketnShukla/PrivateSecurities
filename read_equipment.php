...
$con = new mysqli('localhost', 'root', 'manager', 'secdev', 3307);
    ...
    if(isset($_GET['delete'])){
        $sno = $_GET['delete'];
       $delete=true;
      $sql =  "DELETE FROM `equipment` WHERE `equipment`.`ID` = $sno;";
    ...
    if(isset($_POST['snoEdit'])){
        ...
        $type = $_POST['type'];
        $cid = $_POST['cid']; 
         $sql = "UPDATE `equipment` SET `ID` = '$ID', `Type` = '$type', `Client_ID` = '$cid' WHERE `equipment`.`ID` = '$ID';";
...
<div class="child2">
        <a href="checkequip.php"><button class="btn btn-sm btn-primary">Check Available</button></a>
      </div>
    </div>  
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <div class="container">
      <form action="read_equipment.php" method="post">
          <input type="hidden" name="snoEdit" id="snoEdit">
            <input type="text" name="id" id="id" placeholder="Enter ID">
            <input type="text" name="type" id="type" placeholder="Enter equipment Name" disabled>
    ...
    deletes = document.getElementsByClassName('delete');
        Array.from(deletes).forEach((element) => {
            ...
            if(confirm("Are you sure you want to delete")){
                window.location = `/dmp/read_equipment.php?delete=${sno}`;
